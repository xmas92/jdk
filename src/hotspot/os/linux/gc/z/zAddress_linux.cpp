...
static size_t probe_heap_base_max_shift() {
  ...
  if (max_heap_base_shift == 0) {
    ...
    if (result_addr != MAP_FAILED) {
      max_heap_base_shift = BitsPerSize_t - count_leading_zeros((size_t) result_addr) - 1;
      munmap(result_addr, page_size);
    }
  }

  log_debug_p(gc, init)("Probing address space for the highest valid bit: %zu", max_heap_base_shift);
