...
inline uint VtableStubs::unsafe_hash(address entry_point) {
  ...
  address is_vtable_stub_addr = vtable_stub_addr + offset_of(VtableStub, _is_vtable_stub);
  address vtable_index_addr = vtable_stub_addr + offset_of(VtableStub, _index);
  bool is_vtable_stub = [&]() {
    // Assumes that object representation of false is all 0 bits.
    char t[sizeof(bool)];
    memcpy(t, is_vtable_stub_addr, sizeof(t));
