...
inline void MarkSweep::adjust_pointer(T* p) {
  ...
  if (!CompressedOops::is_null(heap_oop)) {
    ...
    if (SlidingForwarding::is_forwarded(obj)) {
      oop new_obj = SlidingForwarding::forwardee<MODE>(obj);
...
template <typename T>
void AdjustPointerClosure<MODE>::do_oop_work(T* p)           { MarkSweep::adjust_pointer<MODE>(p); }
template <SlidingForwarding::ForwardingMode MODE>
inline void AdjustPointerClosure<MODE>::do_oop(oop* p)       { do_oop_work(p); }
