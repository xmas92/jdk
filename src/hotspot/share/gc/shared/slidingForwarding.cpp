...
void SlidingForwarding::initialize(MemRegion heap, size_t region_size_words) {
  ...
  if (UseAltGCForwarding) {
    ...
    guarantee(forwarding_mode() != ForwardingMode::BIASED_BASE_TABLE || (_heap_start_region_bias << _region_size_bytes_shift) == (uintptr_t)_heap_start, "must be aligned: _heap_start_region_bias: " SIZE_FORMAT ", _region_size_byte_shift: %u, _heap_start: " PTR_FORMAT, _heap_start_region_bias, _region_size_bytes_shift, p2i(_heap_start));
...
#ifdef _LP64
  if (UseAltGCForwarding && forwarding_mode() == ForwardingMode::BIASED_BASE_TABLE) {
    assert(_bases_table == nullptr, "should not be initialized yet");
    assert(_fallback_table == nullptr, "should not be initialized yet");
