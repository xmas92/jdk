...
oop ZObjArrayAllocator::initialize(HeapWord* mem) const {
  ...
  const bool element_is_reference_type = is_reference_type(element_type);
  ...
  if (!is_aligned(base_offset_in_bytes, BytesPerWord)) {
    ...
    assert(!element_is_reference_type, "Must be a TypeArray");
  ...
  auto initialize_memory = [&]() {
    for (size_t processed = 0; processed < aligned_payload_size; processed += segment_max) {
      ...
      const uintptr_t fill_value = element_is_reference_type ? colored_null : 0;
      ...
      if (element_is_reference_type && !seen_gc_safepoint && gc_safepoint_happened()) {
