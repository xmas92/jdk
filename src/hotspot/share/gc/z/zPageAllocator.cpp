...
class ZSegmentStash {
private:
  ZGranuleMap<zoffset>* const _physical_mappings;
...
public:
  ZMemoryAllocationData()
    : _claimed_mappings(1),
      ...
      _multi_numa_allocations(nullptr),
...
ZMemoryAllocationData::~ZMemoryAllocationData() {
  if (_multi_numa_allocations != nullptr) {
    _multi_numa_allocations->~ZArray<ZMemoryAllocation>();
    ...
    FREE_C_HEAP_ARRAY(ZArray<ZMemoryAllocation>, _multi_numa_allocations);
  }
}
...
void ZMemoryAllocationData::reset_for_retry() {
  ...
  // Clear multi numa allocations and mappings,
  // but do not deallocate, we will more than
  // likely be a multi numa allocation the next
  // time around.
  if (_multi_numa_allocations != nullptr) {
...
void ZMemoryAllocationData::set_multi_numa_allocation() {
  ...
  if (_multi_numa_allocations == nullptr) {
    ...
    // ZArray<ZMemoryAllocation> _is_multi_numa_allocations;
    void* const multi_numa_allocations_memory = NEW_C_HEAP_OBJ(ZArray<ZMemoryAllocation>, mtGC);
    _multi_numa_allocations = ::new (multi_numa_allocations_memory) ZArray<ZMemoryAllocation>(length);
...
public:
  static void install_tracker(const ZPageAllocation* allocation, ZPage* page) {
    ...
    log_debug(gc, page)("Created Multi-NUMA Allocation, size: %zu, num: %d",
                        allocation->size(), partial_allocations->length());
    ...
    for (const ZMemoryAllocation* partial_allocation; iter.next_addr(&partial_allocation);) {
      ...
      log_debug(gc, page)(" MMA Segment, size: %zu, node: %d",
                          partial_vmem.size(), numa_id);
    }
  ...
  static void free_and_destroy(ZPageAllocator* allocator, ZPage* page) {
    ...
    log_debug(gc, page)("Free Multi-NUMA Allocation, size: %zu, num: %d",
                        page->size(), tracker->map()->length());
    ...
    for (Element partial_allocation{}; iter.next(&partial_allocation);) {
      ...
      if (remaining_vmem.size() != 0) {
        // Failed to get vmem for all memory, uncommit and free the remaining
    ...
    // Free the page's virtual memory
    allocator->free_virtual(page->virtual_memory());
    for (int numa_id = 0; numa_id < numa_nodes; ++numa_id) {
      PerNUMAData& numa_data = per_numa_mappings[numa_id];
      log_debug(gc, page)(" Reinsert MMA Segments, node: %d, mapped: %zu, uncommitted: %zu",
                          numa_id, numa_data._mapped, numa_data._uncommitted);
    }

    {
      ...
      for (int numa_id = 0; numa_id < numa_nodes; ++numa_id) {
        ...
        for (ZMemoryRange mapping{}; iter.next(&mapping);) {
          state.cache()->insert_mapping(mapping);
        }
      }
...
bool ZPageAllocator::claim_mapped_or_increase_capacity(ZMemoryAllocation* allocation, ZCacheState& state) {
...
bool ZPageAllocator::claim_physical(ZMemoryAllocation* allocation, ZCacheState& state) {
  const size_t size = allocation->size();
  ...
  if (!claim_mapped_or_increase_capacity(allocation, state)) {
...
bool ZPageAllocator::claim_physical_multi_numa(ZPageAllocation* allocation) {
  ...
  const auto do_claim_each_node = [&](auto get_alloc_size) {
    ...
    do {
      ...
      if (!claim_physical(partial_allocation, state)) {
        // Claiming failed
...
bool ZPageAllocator::claim_physical_round_robin(ZPageAllocation* allocation) {
  ...
  do {
    ...
    if (claim_physical(memory_allocation, state)) {
      // Success
...
bool ZPageAllocator::claim_virtual_memory_multi_numa(ZPageAllocation* allocation) {
  ...
  for (int id = 0; id < numa_nodes; ++id) {
    ZMemoryRange vmem = _virtual.alloc(size, id, false /* force_low_address */);
    if (vmem.size() == size) {
...
retry:
  ...
  // we're done.
  if (is_alloc_satisfied(allocation)) {
    ZMemoryRange vmem = allocation->pop_final_mapping();
