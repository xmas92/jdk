  ...
  for (size_t i = 0; i < NumPartitions; i++) {
    while (partition_finger[i] != partition_end[i]) {
      ...
      if (partition_index == i) {
        ...
        ::swap(page, pages->at(partition_finger[partition_index]++));
      }
    }
  }
}
...
int ZRelocationSetSelectorGroup::select_inner(ZPageAge age) {
  ...
  log_trace(gc, reloc)("Relocation Set (%s Pages, %u Age): %d->%d, %d skipped, %zu forwarding entries",
...
void ZRelocationSetSelectorGroup::calculate_fragmentation_limits() {
  ...
  for (uint i = 0; i <= ZPageAgeMax; ++i) {
    ...
    _page_fragmentation_limit[i] = static_cast<size_t>(_page_size * _fragmentation_limit[i] / 100.);
  }
}
...
ZRelocationSetSelector::ZRelocationSetSelector(ZGenerationId id, bool promote_all)
  : _small("Small", ZPageType::small, ZPageSizeSmall, ZObjectSizeLimitSmall, id, promote_all),
    _medium("Medium", ZPageType::medium, ZPageSizeMedium, ZObjectSizeLimitMedium, id, promote_all),
    _large("Large", ZPageType::large, 0 /* page_size */, 0 /* object_size_limit */, id, promote_all),
    _empty_pages() {}
