...
template <typename S>
uintptr_t ZValueStorage<S>::alloc(zbytes size) {
  ...
  const zbytes block_alignment = Offset;
  const zbytes block_size = Offset * S::count();
...
inline zbytes ZContendedStorage::alignment() {
...
inline zbytes ZPerCPUStorage::alignment() {
  return to_zbytes(sizeof(uintptr_t));
}
...
inline zbytes ZPerNUMAStorage::alignment() {
  return to_zbytes(sizeof(uintptr_t));
}
...
inline zbytes ZPerWorkerStorage::alignment() {
  return to_zbytes(sizeof(uintptr_t));
}
...
inline ZValue<S, T>::ZValue()
  : _addr(S::alloc(to_zbytes(sizeof(T)))) {
