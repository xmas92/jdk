...
class CallbackWrapper : public StackObj {
 ...
 public:
  CallbackWrapper(JvmtiTagMap* tag_map, oop o) {
    ...
    _klass_tag = tag_for(tag_map, _o->klass()->java_mirror_no_keepalive());
  }
 ...
 public:
    ...
    CallbackWrapper(tag_map, o)
  {
    ...
    if (_is_reference_to_self) {
      ...
      _referrer_klass_tag = tag_for(tag_map, _referrer->klass()->java_mirror_no_keepalive());
    }
  }
   ...
   void* user_data)
{
  ...
  for (int i=0; i<field_map->field_count(); i++) {
    ...
    address addr = cast_from_oop<address>(klass->java_mirror_no_keepalive()) + offset;
    jvalue value;
...
void IterateThroughHeapObjectClosure::do_object(oop obj) {
  ...
  // skip if object is a dormant shared object whose mirror hasn't been loaded
  if (obj != nullptr &&   obj->klass()->java_mirror_no_keepalive() == nullptr) {
...
inline bool VM_HeapWalkOperation::iterate_over_array(oop o) {
  ...
  oop mirror = ObjArrayKlass::cast(array->klass())->java_mirror_no_keepalive();
...
inline bool VM_HeapWalkOperation::iterate_over_class(oop java_class) {
  ...
  if (klass->is_instance_klass()) {
    ...
    // get the java mirror
    oop mirror = klass->java_mirror_no_keepalive();
    ...
    if (java_super != nullptr && java_super != vmClasses::Object_klass()) {
      oop super = java_super->java_mirror_no_keepalive();
    ...
    // class loader
    oop cl = ik->class_loader_no_keepalive();
    if (cl != nullptr) {
    ...
    // protection domain
    oop pd = ik->protection_domain_no_keepalive();
    if (pd != nullptr) {
    ...
    // signers
    oop signers = ik->signers_no_keepalive();
    if (signers != nullptr) {
      ...
      for (int i = 1; i < pool->length(); i++) {
        ...
        if (tag.is_string() || tag.is_klass() || tag.is_unresolved_klass()) {
          ...
          if (tag.is_string()) {
            ...
            entry = pool->resolved_klass_at(i)->java_mirror_no_keepalive();
          } else {
            ...
            entry = klass->java_mirror_no_keepalive();
          }
    ...
    for (i = 0; i < interfaces->length(); i++) {
      oop interf = interfaces->at(i)->java_mirror_no_keepalive();
      if (interf == nullptr) {
        continue;
      }
...
inline bool VM_HeapWalkOperation::iterate_over_object(oop o) {
  ...
  if (!CallbackInvoker::report_class_reference(o, o->klass()->java_mirror_no_keepalive())) {
    return false;
  }
