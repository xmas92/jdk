 ...
 protected:
  ...
  virtual void handle_oop_offset() {
    ...
    if (loc == NULL) {
      ...
      _fr.print_on(tty);
      assert(loc != NULL, "missing register map entry reg: " INT32_FORMAT_X_0 " %s loc: " INTPTR_FORMAT, reg->value(), reg->name(), p2i(loc));
    }
  #endif
    _f->do_oop(loc);
  }
  ...
  if (is_interpreted_frame()) {
    ...
    { // mark arguments (see nmethod::print_nmethod_labels)
      ...
      for (SignatureStream ss(m->signature()); !ss.at_return_type(); ) {
        ...
        if (fst->is_stack()) {
          assert(((int)fst->reg2stack()) >= 0, "reg2stack: " INT32_FORMAT_X_0, fst->reg2stack());
