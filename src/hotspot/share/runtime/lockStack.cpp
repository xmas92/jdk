...
LockStack::LockStack(JavaThread* jt) :
  _top(lock_stack_base_offset), _data() {
  _data[0] = nullptr;
#ifdef ASSERT
  ...
  if (SafepointSynchronize::is_at_safepoint() || (Thread::current()->is_Java_thread() && is_owning_thread())) {
    ...
    for (int i = 0; i < top; i++) {
      ...
      for (int j = i + 1; j < top; j++) {
        assert(base()[i] != base()[j], "entries must be unique: %s", msg);
      }
    }
    for (int i = top; i < CAPACITY; i++) {
      assert(base()[i] == nullptr, "only zapped entries from next_slot onward: i: %d, top: %d, entry: " PTR_FORMAT, i, top, p2i(base()[i]));
    }
  }
}
#endif
