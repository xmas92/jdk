...
void MonitorList::update_count(size_t unlink_count) {
  Atomic::sub(&_count, unlink_count);
}
...
void MonitorList::Unlinker::check_head_change() {
  if (_prev == nullptr && !_observed_head_change && _head != Atomic::load(&_list._head)) {
    _observed_head_change = true;
    _prev = find_prev_on_head_change();
  }
}

ObjectMonitor* MonitorList::Unlinker::find_prev_on_head_change() const {
  Iterator iter = _list.iterator();
  for (;;) {
    assert(iter.has_next(), "_head must be reachable");
...
void MonitorList::Unlinker::unlink_batch() {
  ...
  const auto unlink_reachable_from = [&](ObjectMonitor* from){
    Iterator iter(from);
    while (iter.has_next()) {
      if (iter.next() == unlink) {
        return true;
...
MonitorList::Unlinker::Unlinker(MonitorList& list) :
  _list(list),
  _unlink_count(0),
  _batch_count(0),
...
void MonitorList::Deleter::delete_one_monitor() {
  ObjectMonitor* next = _next;
  _next = _next->next_delete_om();
  delete next;
  _delete_count++;
  assert(_delete_count <= _size, "must be");
}

#ifdef ASSERT
ObjectMonitor* MonitorList::Deleter::Iterator::next() {
  ObjectMonitor* current = _current;
  _current = current->next_delete_om();
  return current;
}
#endif
...
public:
  ...
  void before_handshake(size_t deflated_and_unlinked_count) {
    if (_stream != nullptr) {
      _timer.stop();
      _stream->print_cr("before handshaking: {deflated,unlinked}_count=%zu"
  ...
  void end(size_t deflated_unlinked_and_deleted_count) {
    if (_stream != nullptr) {
      ...
      if (deflated_unlinked_and_deleted_count != 0 || _debug.is_enabled()) {
        _stream->print_cr("{deflated,unlinked,deleted}_count=%zu monitors in %3.7f secs",
...
#ifdef ASSERT
    if (UseObjectMonitorTable) {
      MonitorList::Deleter::Iterator iter = deleter.iterator();
      while (iter.has_next()) {
        assert(!LightweightSynchronizer::contains_monitor(current, iter.next()), "Should have been removed");
        if (SafepointMechanism::should_process(current)) {
          ThreadBlockInVM tbivm(current);
        }
      }
...
#endif
    log.before_handshake(deleter.size());
