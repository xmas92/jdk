...
public:
  ...
  private:
    GrowableArrayCHeap<RBNode*, mtNMT> _to_visit;
    const RBTree* const                    _tree;
    RBNode*                                _next;

  public:
    NodeIteratorImpl(RBTree* tree) : _tree(tree) {
      if (_tree->_num_nodes == 0) {
        _next = nullptr;
        return;
      }

      RBNode* head = _tree->_root;
      while (head != nullptr) {
        _to_visit.push(head);
        head = Forward ? head->_left : head->_right;
      }

      _next = _to_visit.pop();
    }

    bool next(RBNode** elem) {
      if(_next == nullptr) {
        return false;
      }
      ...
      *elem = _next;
      ...
      _next = Forward ? _next->_right : _next->_left;
      while (_next != nullptr) {
        _to_visit.push(_next);
        _next = Forward ? _next->_left : _next->_right;
      }
