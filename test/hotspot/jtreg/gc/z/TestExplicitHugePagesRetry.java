...
public class TestExplicitHugePagesRetry {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (!config.supportsStaticHugePages()) {
            throw new SkippedException("Persistent Huge Pages are not configured and/or supported.");
        }
        long persistentHugePageSize = config.getStaticDefaultHugePageSize();
        HugePageConfiguration.StaticHugePageStats stats = HugePageConfiguration.readStaticHugePageStatsFromOS();
        ...
        for (int i = 0; i < ITERATIONS; i++) {
            ...
            System.out.println(HugePageConfiguration.readStaticHugePageStatsFromOS());
            System.out.println();
